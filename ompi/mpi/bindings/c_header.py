...
# This script lives alongside the ompi_bindings package (the main bindings
# generator), so importing shared domain knowledge from it works both for
# in-tree and VPATH builds: when this file is run as a script, its own
# directory is placed on sys.path[0], and ompi_bindings/ is a sibling.
from ompi_bindings import consts

# ============================= Constants / Globals ============================
ABI_INTERNAL = "_ABI_INTERNAL"

# Types whose names get an "_ABI_INTERNAL" suffix appended in the mangled
# header.  The opaque C handle types are shared with the main bindings
# generator via consts.C_HANDLES so the two generators cannot drift; the
# remaining entries (MPI_Status, the callback typedefs, the MPI_T types, and
# the two deprecated attribute-function typedefs) are specific to this
# header-mangling pass.
INTERNAL_DATATYPES = list(consts.C_HANDLES) + [
    "MPI_Status",
...
# functions/constants are deprecated (see the comment near its use, below).
...
def cb_declaration(proc_expression, mangle_names):
...
def parse_args():
    ...
    parser.add_argument("--abi-json", type=str, help="path to ABI JSON file [./]")
...
def main():
    ...
    # Match lines that start with `$CATEGORY:`. Any amount of whitespace is
    # allowed in the case of indenting. However, only preceding whitespace is
    # allowed so that commented-out lines are NOT included.
    ...
    for line in lines:
        ...
        # If there's no match, category is None. Then we just want to preserve
        # the line as-is.
        if category:
    ...
    for proc in std.CALLBACKS.values():
        output.append(cb_declaration(proc.express.iso_c, MANGLE_NAMES))
        if proc.has_embiggenment():
            output.append(cb_declaration(proc.express.embiggen.iso_c, MANGLE_NAMES))
