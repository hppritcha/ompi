...
def _source_exists(srcdir, stem):
    ...
    candidates = (
        ...
        c_dir / (stem + "_abi.c"),
        c_dir / (stem + ".c.in"),
        # A handful of bindings (currently the MPI_Type_get_contents /
        # MPI_Type_get_envelope pair, plain and large-count) are generated
        # from templates with the ".c.in_nbc" (no big count) / ".c.in_obc"
        # (only big count) suffixes rather than ".c.in".  They still emit
        # <stem>_abi_generated.c into libmpi_abi, so treat them as evidence of
        # a C ABI binding.
        c_dir / (stem + ".c.in_nbc"),
        c_dir / (stem + ".c.in_obc"),
