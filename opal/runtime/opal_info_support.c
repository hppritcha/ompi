...
OBJ_CLASS_INSTANCE(opal_info_component_map_t, opal_list_item_t, component_map_construct,
...
int opal_info_init(int argc, char **argv, opal_cmd_line_t *opal_info_cmd_line)
{
    ...
    if (OPAL_SUCCESS != (ret = opal_init_util(&argc, &argv))) {
        opal_show_help("help-opal_info.txt", "lib-call-fail", true, "opal_init_util", __FILE__,
                       __LINE__, NULL);
        exit(ret);
    }
    ...
    opal_cmd_line_make_opt3(
        opal_info_cmd_line, '\0', NULL, "param", 2,
    ...
    opal_cmd_line_make_opt3(opal_info_cmd_line, '\0', NULL, "params", 2, "Synonym for --param");
    opal_cmd_line_make_opt3(opal_info_cmd_line, '\0', NULL, "internal", 0,
                            "Show internal MCA parameters (not meant to be modified by users)");
    opal_cmd_line_make_opt3(
        opal_info_cmd_line, '\0', NULL, "path", 1,
    ...
    opal_cmd_line_make_opt3(
        opal_info_cmd_line, '\0', NULL, "show-failed", 0,
    ...
    } else if (opal_cmd_line_is_taken(opal_info_cmd_line, "parsable")
               || opal_cmd_line_is_taken(opal_info_cmd_line, "parseable")) {
...
static int info_register_framework(mca_base_framework_t *framework,
...
int opal_info_register_project_frameworks(const char *project_name,
    ...
    if (want_all) {
        ...
        for (i = 0; i < count; ++i) {
            ...
            if (0 == strcmp(opal_info_path_prefix, scope)) {
                ...
                opal_info_show_path(opal_info_path_sharedstatedir,
                         ...
                         opal_pointer_array_t *component_map, opal_cmd_line_t *opal_info_cmd_line)
{
    ...
    if (want_all) {
        ...
        for (i = 0; i < count; ++i) {
            ...
            opal_info_show_component_version(mca_types, component_map, type, component,
                                             opal_info_ver_full, opal_info_ver_all);
...
void opal_info_err_params(opal_pointer_array_t *component_map)
{
    ...
    for (i = 0; i < component_map->size; i++) {
        if (NULL
            == (mptr = (opal_info_component_map_t *) opal_pointer_array_get_item(component_map,
                                                                                 i))) {
            continue;
        }
        map = mptr;
...
static void opal_info_show_mca_group_params(const mca_base_var_group_t *group,
                                            mca_base_var_info_lvl_t max_level, bool want_internal)
{
    ...
    if (0 != strcmp(group_component, "base")) {
        ...
        if (0 <= var_id) {
            ...
            if (NULL != value && NULL != value->stringval && '\0' != value->stringval[0]) {
                mca_base_component_parse_requested(value->stringval, &include_mode,
    ...
    for (i = 0; i < count; ++i) {
        ...
        if (OPAL_SUCCESS != ret || ((var->mbv_flags & MCA_BASE_VAR_FLAG_INTERNAL) && !want_internal)
            || max_level < var->mbv_info_lvl) {
            continue;
        }

        if (opal_info_pretty && curr_group != group) {
            opal_asprintf(&message, "MCA%s %s%s", requested ? "" : " (-)", group->group_framework,
    ...
    for (i = 0; i < count; ++i) {
        ...
        ret = mca_base_pvar_dump(variables[i], &strings,
                                 !opal_info_pretty ? MCA_BASE_VAR_DUMP_PARSABLE
                                                   : MCA_BASE_VAR_DUMP_READABLE);
        if (OPAL_SUCCESS != ret) {
            continue;
...
void opal_info_out(const char *pretty_message, const char *plain_message, const char *value)
{
    ...
    if (opal_info_pretty && NULL != pretty_message) {
        if (centerpoint > (int) strlen(pretty_message)) {
            opal_asprintf(&spaces, "%*s", centerpoint - (int) strlen(pretty_message), " ");
        } else {
            spaces = strdup("");
#if OPAL_ENABLE_DEBUG
            if (centerpoint < (int) strlen(pretty_message)) {
                opal_show_help("help-opal_info.txt", "developer warning: field too long", false,
 ...
 */
void opal_info_out_int(const char *pretty_message, const char *plain_message, int value)
{
    char *valstr;
...
void opal_info_show_component_version(opal_pointer_array_t *mca_types,
                                      opal_pointer_array_t *component_map, const char *type_name,
 ...
 */
void opal_info_show_mca_version(const mca_base_component_t *component, const char *scope,
                                const char *ver_type)
{
    bool printed;
    bool want_mca = false;
    ...
    if (0 == strcmp(ver_type, opal_info_ver_all) || 0 == strcmp(ver_type, opal_info_ver_mca)) {
        want_mca = true;
    }

    if (0 == strcmp(ver_type, opal_info_ver_all) || 0 == strcmp(ver_type, opal_info_ver_type)) {
        want_type = true;
    }
    ...
    if (opal_info_pretty) {
        ...
        opal_asprintf(&message, "mca:%s:%s:version", component->mca_type_name,
...
char *opal_info_make_version_str(const char *scope, int major, int minor, int release,
                                 const char *greek, const char *repo)
{
    ...
    temp[BUFSIZ - 1] = '\0';
    if (0 == strcmp(scope, opal_info_ver_full) || 0 == strcmp(scope, opal_info_ver_all)) {
