...
{
    mca_accelerator_ze_check_addr,

    mca_accelerator_ze_create_stream,

    mca_accelerator_ze_create_event,
    mca_accelerator_ze_record_event,
    mca_accelerator_ze_query_event,

    mca_accelerator_ze_memcpy_async,
    mca_accelerator_ze_memcpy,
    mca_accelerator_ze_memmove,
    mca_accelerator_ze_mem_alloc,
    mca_accelerator_ze_mem_release,
    mca_accelerator_ze_get_address_range,

    mca_accelerator_ze_host_register,
    mca_accelerator_ze_host_unregister,

    mca_accelerator_ze_get_device,
    mca_accelerator_ze_device_can_access_peer,

    mca_accelerator_ze_get_buffer_id
};
                                       ...
                                       opal_accelerator_transfer_type_t type)
{
    ...
    if (type == MCA_ACCELERATOR_TRANSFER_DTOH && size <= opal_accelerator_ze_memcpyD2H_limit) {
        memcpy(dest, src, size);
        return OPAL_SUCCESS;
    }

    if (type == MCA_ACCELERATOR_TRANSFER_HTOD && size <= opal_accelerator_ze_memcpyH2D_limit) {
        memcpy(dest, src, size);
        return OPAL_SUCCESS;
    }

    if (opal_accelerator_ze_memcpy_async) {
        ...
        err = hipStreamSynchronize(opal_accelerator_ze_MemcpyStream);
        if (hipSuccess != err ) {
            opal_output_verbose(10, opal_accelerator_base_framework.framework_output,
