                                ...
                                size_t* cd, ompi_datatype_t** d, int32_t* type)
{
    ...
    case 1:     /* GET THE ARGUMENTS */
        if(*ci < pArgs->ci || *cl < pArgs->cl || *ca < pArgs->ca || *cd < pArgs->cd) {
            return MPI_ERR_ARG;
        }
                                                                        ...
                                                                        const struct ompi_proc_t* remote_processor )
{
    ...
    if (need_swap) {
        for (i = 0 ; i < number_of_ints ; ++i) {
            array_of_ints[i] = opal_swap_bytes4(array_of_ints[i]);
        }
        for (i = 0 ; i < number_of_counts ; ++i) {
            array_of_counts[i] = opal_swap_bytes8(array_of_counts[i]);
        }
