...
char *opal_find_absolute_path(char *app_name)
{
    ...
    } else if ('.' == app_name[0] || NULL != strchr(app_name, OPAL_PATH_SEP[0])) {
...
again:
    ...
    if (-1 == fsrc && -1 == vfsrc) {
        ...
        OPAL_OUTPUT_VERBOSE((10, 0,
                             "opal_path_nfs: stat(v)fs on file:%s failed errno:%d directory:%s\n",
                             fname, errno, file));
        if (EPERM == errno) {
            free(file);
            if (NULL != ret_fstype) {
        ...
        if (NULL == last_sep || (1 == strlen(last_sep) && OPAL_PATH_SEP[0] == *last_sep)) {
            free(file);
            if (NULL != ret_fstype) {
                *ret_fstype = NULL;
            }
        ...
        if (NULL != fs_type) {
            for (x = 0; x < FS_TYPES_NUM; x++) {
                ...
                if (0 == strcasecmp(fs_types[x].f_fsname, fs_type)) {
                    OPAL_OUTPUT_VERBOSE(
                        (10, 0, "opal_path_nfs: file:%s on fs:%s\n", fname, fs_type));
                    free(fs_type);
                    if (NULL != ret_fstype) {
...
int opal_path_df(const char *path, uint64_t *out_avail)
{
    int rc = -1;
    int trials = 5;
    int err = 0;
