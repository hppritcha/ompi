...
class ABIConverterBuilder:
    ...
    def generate_new_datatype_convert_fn(self):
        ...
        for i,value_name in enumerate(consts.PREDEFINED_OPTIONAL_FORTRAN_DATATYPES):
            intern_name = self.mangle_name(value_name)
            # Emit these optional-Fortran-datatype branches UNCONDITIONALLY.
            # Every symbol referenced here is always defined: the native handle
            ...
            # Unguarded, an unavailable kind converts to its sentinel object,
            # whose size is MPI_UNDEFINED -- matching MPI-5.0 section 20.4.
            lines.append('} else if (%s == datatype) {' % (intern_name))
            lines.append(f'return {value_name};')
        lines.append('}')
    ...
    def generate_new_datatype_convert_fn_intern_to_abi(self):
        ...
        for i,value_name in enumerate(consts.PREDEFINED_OPTIONAL_FORTRAN_DATATYPES):
            intern_name = self.mangle_name(value_name)
            # Emit unconditionally, symmetric with the abi->intern converter
            # above -- see the detailed rationale there for why neither
            # OMPI_HAVE_FORTRAN_<kind> nor OMPI_BUILD_FORTRAN_BINDINGS may guard
            # this block.  Each unavailable kind is a distinct sentinel object,
            # so this reverse mapping is unambiguous.
            lines.append('} else if (%s == datatype) {' % (value_name))
            lines.append(f'return {intern_name};')
        lines.append('}')
